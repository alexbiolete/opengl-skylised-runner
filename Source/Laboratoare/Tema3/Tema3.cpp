...
#define PYRAMID_COUNT					10

// Time-related values
#define PRINT_FUEL_DELTA				1
#define RESET_TRIGGER_DELTA				1
...
void Tema3::Init()
{
	...
	const string texture_location = "Source/Laboratoare/Tema3/Textures/";
	...
	// Initialization of triggers for special platform effect activation and score
	{
		obstacle_trigger1 = false;
		obstacle_trigger2 = true;
		collectible_trigger1 = false;
		collectible_trigger2 = true;
	}
	...
	// HUD
	{
		...
		Mesh* speedBar = Object2D::CreateRectangle("speedBar", corner, HUD_BAR_X, HUD_BAR_Y, color_green, true);
		AddMeshToList(speedBar);
		Mesh* life = Object2D::CreateCircle("life", corner, HUD_INDICATOR_RADIUS, color_orange, true);
		AddMeshToList(life);
	}
	...
	// Platform object
	{
		...
		// Load textures
		{
			...
			texture->Load2D((texture_location + "aztec_pavement.jpg").c_str(), GL_REPEAT);
			mapTextures["aztec_pavement"] = texture;
		}
		...
		for (int i = 5; i < PLATFORM_COUNT; i++) {
			...
			GLint random_filler = rand() % 10;
			if (random_filler == 0) {
				platform[i].has_obstacle = true;
				platform[i].has_collectible = false;
			} 
			else if (random_filler == 5) {
				platform[i].has_obstacle = false;
				platform[i].has_collectible = true;
			}
			else {
				platform[i].has_obstacle = false;
				platform[i].has_collectible = false;
	...
	// Player object
	{
		...
		{
			Texture2D* texture = new Texture2D();
			texture->Load2D((texture_location + "aztec_calendar.jpg").c_str(), GL_REPEAT);
			mapTextures["aztec_calendar"] = texture;
		}
	...
	// Skybox objects
	{
		...
		{
			Texture2D* texture = new Texture2D();
			texture->Load2D((texture_location + "grass.jpg").c_str(), GL_REPEAT);
			mapTextures["grass"] = texture;
		}

		for (int i = 0; i < BACKGROUND_COUNT; i++) {
			background[i].position = glm::vec3(0, 0, i * BACKGROUND_SIZE);
		}
	}

	// Pyramid decorative object
	{
		// Drawing 3D object
		{
			float height = 2.0f * sqrt(2.0f / 3.0f);
		...
		{
			Texture2D* texture = new Texture2D();
			texture->Load2D((texture_location + "aztec_pyramid.jpg").c_str(), GL_REPEAT);
			mapTextures["aztec_pyramid"] = texture;
		}

		// Initialization of pyramids
		for (int i = 0; i < PYRAMID_COUNT; i++) {
			srand(time(NULL) + i);
			
			pyramid[i].scale = rand() % 10;
			
			GLint side_randomizer = rand() % 2;
			if (side_randomizer == 0)
				pyramid[i].position.x = 20;
			else
				pyramid[i].position.x = -10;

			pyramid[i].position.z = 50 + 100 * (rand() % 10);
		}
	}
	...
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((texture_location + "spike_wall.jpg").c_str(), GL_REPEAT);
		mapTextures["spike_wall"] = texture;
	}
	// Load textures
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((texture_location + "collectible.gif").c_str(), GL_REPEAT);
		mapTextures["collectible"] = texture;
	}
}
...
void Tema3::drawHud()
{
	if (game.first_person_camera) {
		// Life count indicators
		{
			for (int i = 0; i < game.lives; i++) {
				...
				modelMatrix *= Transform3D::Translate(cameraPosition.x / 15 + i * 0.25f + 0.075f, cameraPosition.y / 1.125f, cameraPosition.z + 5.01f);
		...
		// Speed bar
		{
			...
			modelMatrix *= Transform3D::Translate(cameraPosition.x / 0.51805f, cameraPosition.y / 1.125f, cameraPosition.z + 5);
			modelMatrix *= Transform3D::RotateOX(RADIANS(30));
		...
		// Life count indicators
		{
			for (int i = 0; i < game.lives; i++) {
				...
				modelMatrix *= Transform3D::Translate(cameraPosition.x / 0.5175f - i * 0.25f, cameraPosition.y / 1.185f, cameraPosition.z + 5.01f);
				modelMatrix *= Transform3D::RotateOX(RADIANS(30));
	...
	// Score
	{
		Text->RenderTextNumber("Score: ", game.score, resolution.x / 1.175f, resolution.y / 17.5, 1.0f, color_white);
	}

	// Text
	{
		if (sphere.mapFalling && game.lives > 0 && !game.lifeReposition) {
			Text->RenderText("YOU FELL!", resolution.x / 2.1f, resolution.y / 2.2f, 1.0f, glm::vec3(1, 1, 1));
			Text->RenderText("Press ENTER to continue in exchange of a life.", resolution.x / 3.25f, resolution.y / 2, 1.0f, color_white);
		}
		else if (((sphere.movementSpeed <= 0) || sphere.mapFalling) && game.lives <= 0) {
			Text->RenderText("GAME OVER!", resolution.x / 2.15f, resolution.y / 2.2f, 1.0f, glm::vec3(1, 1, 1));
			Text->RenderText("Press ENTER to start again.", resolution.x / 2.6f, resolution.y / 2, 1.0f, color_white);
		}
	}
}
...
void Tema3::Update(float deltaTimeSeconds)
{
	...
	// https://stackoverflow.com/questions/10807681/loop-every-10-second
	{
		...
		if (time_counter1 > (GLdouble)(RESET_TRIGGER_DELTA * CLOCKS_PER_SEC)) {
			obstacle_trigger2 = true;
			collectible_trigger2 = true;

			time_counter1 -= (GLdouble)(RESET_TRIGGER_DELTA * CLOCKS_PER_SEC);
		}
	}
	...
	/* {
		for (int i = 0; i < BACKGROUND_COUNT; i++) {
			// Move the skybox that is behind the camera at the end of the map
			//if (background[i].position.z < cameraPosition.z) {
				//background[i].position.z += BACKGROUND_SIZE;
			//}
			...
			// Model
			{
				...
				modelMatrix *= Transform3D::Translate(background[i].position.x, -0.5f, background[i].position.z);
				modelMatrix *= Transform3D::Scale(BACKGROUND_SIZE, 0.1f, BACKGROUND_SIZE);

				RenderTexturedMesh(meshes["box"], shaders["TextureShader"], modelMatrix, mapTextures["grass"]);
			}
		}
	} */
	...
	// Camera mode
	{
		if (game.first_person_camera) {
			...
			lightPosition = glm::vec3(CENTER_X_COORDINATE, 0, 0) - glm::vec3(0, 0, 3);
		}
	}
	...
	// Sphere
	{
		...
		// Model
		{
			...
			RenderTexturedMesh(meshes["sphere"], shaders["TextureShader"], modelMatrix, mapTextures["aztec_calendar"]);
		}
	}
	...
	// Platforms
	{
		for (int i = 0; i < PLATFORM_COUNT; i++) {
			...
			// Model
			{
				...
				RenderTexturedMesh(meshes["box"], shaders["TextureShader"], modelMatrix, mapTextures["aztec_pavement"]);
			}

			if (platform[i].has_obstacle) {
				// Model
				{
					if (obstacle_trigger2 && intersects(sphere, platform[i])) {
						obstacle_trigger1 = true;
						obstacle_trigger2 = false;
					}

					if (obstacle_trigger1) {
						if (game.lives >= 0) {
							game.lives--;
						}
						platform[i].has_obstacle = false;
					...
					modelMatrix *= Transform3D::Translate(platform[i].position.x, SURFACE_Y_OFFSET + 0.5f, platform[i].position.z);
					modelMatrix *= Transform3D::Scale(2, 1, 0.5f);

					RenderTexturedMesh(meshes["box"], shaders["TextureShader"], modelMatrix, mapTextures["spike_wall"]);
				}
			}
			else if (platform[i].has_collectible) {
				// Model
				{
					collectible_rotation += 0.001f;

					if (collectible_trigger2 && intersects(sphere, platform[i])) {
						collectible_trigger1 = true;
						collectible_trigger2 = false;
					}

					if (collectible_trigger1) {
						game.score += 100;
						platform[i].has_collectible = false;
						collectible_trigger1 = false;
					}
					...
					modelMatrix *= Transform3D::Translate(platform[i].position.x, SURFACE_Y_OFFSET + 1.25f, platform[i].position.z);
					modelMatrix *= Transform3D::RotateOY(collectible_rotation);
					modelMatrix *= Transform3D::Scale(0.5f, 0.5f, 0.5f);

					RenderTexturedMesh(meshes["box"], shaders["TextureShader"], modelMatrix, mapTextures["collectible"]);
				}
			}
		}
	}
	...
	// Pyramids
	{
		for (int i = 0; i < PYRAMID_COUNT; i++) {
			...
			// Move the platforms that are behind the camera at the end of the map
			if (pyramid[i].position.z < cameraPosition.z) {
				srand(time(NULL) + i);
			...
			// Model
			{
				...
				modelMatrix *= Transform3D::Translate(pyramid[i].position.x, SURFACE_Y_OFFSET, pyramid[i].position.z);
				modelMatrix *= Transform3D::Scale(pyramid[i].scale, pyramid[i].scale, pyramid[i].scale);

				RenderTexturedMesh(meshes["pyramid"], shaders["TextureShader"], modelMatrix, mapTextures["aztec_pyramid"]);
			}
		}
	}
}
...
void Tema3::RenderTexturedMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	if (!mesh || !shader || !shader->GetProgramID())
		return;
...
void Tema3::OnInputUpdate(float deltaTime, int mods)
{
	...
	{
		if (window->KeyHold(GLFW_KEY_W) && !sphere.mapFalling)
		{
		...
		if (window->KeyHold(GLFW_KEY_S) && !sphere.mapFalling)
		{
			if (sphere.movementSpeed > MIN_MOVEMENT_SPEED)
...
// Collision function
GLboolean Tema3::intersects(Sphere sphere, Platform platform)
{
